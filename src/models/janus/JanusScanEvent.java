...
public class JanusScanEvent extends ScanEvent {
	...
	//TODO don't include friends if flag is off
	...
	private void targetGather(){ //janus/target_gather
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSEST,"observer location " + myEntity.getLocation());
			Tracer.write(Tracer.DETECTION,Tracer.COURSEST,"maximum visibility " + maxVisibility + "km");
		}
		maxVisibility = maxVisibility * 5.0;
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSEST,"test visibility range " + maxVisibility + "km");
		}
		Sensor sensor = this.myEntity.getCurrentSensor();
		if (tracing){
			if (sensor == null){
				Tracer.write(Tracer.DETECTION,Tracer.COURSER,"no active sensor ");
			} else {
				Tracer.write(Tracer.DETECTION,Tracer.COURSER,"using sensor " + sensor.getName());
				Tracer.write(Tracer.DETECTION,Tracer.COURSER,"sensor band " + 
						Sensor.SENSOR_BANDS[sensor.getBand()]);
			}
		}
		//TODO change to a hashmap i think
		Vector<EntityDetection> previousEnemies = listPreviousDetections(sensor); 
	...
	private Vector<EntityDetection> listPreviousDetections(Sensor sensor){ // list_previous_detections
		...
		if (sensor != null){
			if (sensor.getBand() == Sensor.BAND_MMR){
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.COURSEST,"mmr sensors do not perform target gather");
				}
				return tempList;
			}
		}
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSE,"compiling list of previous detections ");
			Tracer.write(Tracer.DETECTION,Tracer.COURSE, "could detect " + this.myEntity.getDetectionList().size());
		}
		...
		for (EntityDetection detection: this.myEntity.getDetectionList()){
			...
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"can see " + target.getName() +
						" level " + 
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSE,"used to be able to see " +
	...
	private SortedList listTargets(ObserverEntity observer, double vismax, Sensor sensor ){
		...
		if (target == null){
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.COURSER,"no entities to detect ");
			}
		...
		*/
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSER,"looking for potential targets ");
		}
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSER,"found " + list.size() + " potential targets");
		}
		return list;
	}
	...
	private double testTarget(ObserverEntity observer, 
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSE,"testing " +
					target.getName() + " location " + target.getLocation().toString());
			Tracer.write(Tracer.DETECTION,Tracer.FINE,"platform type " + target.getPlatform().getName() );
		}
		if ( observer.getForce() == target.getForce()){
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"target and observer in the same force " );
			}
			return -1.0;
		}
		if ( target.getForce() == null){
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"target has no valid force, unable to detect " );
			}
			return -1.0;
		}
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.FINE,"target force: " + target.getForce().getName());
		}
		double x = target.getLocation().getX();
		if (x < xmin) {
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"outside visibility box " );
			}
			return -1.0;
		}
		if (x > xmax) {
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.FINE,"range to target " + distance );
		}
		if ( target.isDead()){
			distance = distance * 5.0; // increase range for dead things to put them at the end of the list
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"target is dead, increasing distance " + distance );
			}
		} else { // target is still alive
			if (observer.getSpotTarget() == target) {
				distance = 0.0;
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.FINE,"the observer has been ordered to spot this target " );
				}
			}
			if (target.getPlatform().getFlyerType() != null){
				distance = distance * 0.5;
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.FINE,"target is an aircraft, decreasing range " + distance );
				}
			} else { 
				// not an aircraft
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.FINE,"target is not an aircraft" );
				}
				double realSize = target.getRealSize();
				int contrastClass = target.getContrastClass();
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.FINE,"target actual size " + realSize );
					Tracer.write(Tracer.DETECTION,Tracer.FINE,"target contrast class " + contrastClass );
				}
				if (contrastClass <= 0){
				...
				if (sensor != null){
					if (sensor.getBand() >= Sensor.BAND_OPTICAL1 && 
						...
						if (tracing){
							Tracer.write(Tracer.DETECTION,Tracer.FINE,"initial bars " + bars );
							Tracer.write(Tracer.DETECTION,Tracer.FINE,"threshold value " + value );
						}
						if (bars < value){
							distance = distance * 2;
							if (tracing){
								Tracer.write(Tracer.DETECTION,Tracer.FINE,"probability of detecting this target is quite low" );
								Tracer.write(Tracer.DETECTION,Tracer.FINE,"weighted value set to " + distance );
							}
						} else { //reasonable chance to detect
							if (target.getCurrentSpeed() > 0.0){
								distance = distance * 0.5;
								if (tracing){
									Tracer.write(Tracer.DETECTION,Tracer.FINE,"target is moving" );
									Tracer.write(Tracer.DETECTION,Tracer.FINE,"weighted value set to " + distance );
								}
							}
						}
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.FINE,"target is " + Constants.HOSTILITY_LEVEL[hostility] );
		}
		if (hostility == Constants.HOSTILITY_FRIEND){
			if (this.myScenario.getParameters().getDoSeeOwn() == Constants.DETECT_FRIENDS_ALWAYS){
				...
				if (level >= Constants.OBSERVATION_LEVEL_IDENTIFIED){
					distance = distance * 5;
					if (tracing){
						Tracer.write(Tracer.DETECTION,Tracer.FINE,"target is an identified friend, weighting set to " + distance );
					}
				}
			}
		}
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.FINE,"detection priority " + priority );
		}
		...
		if (detectionLevel >= Constants.OBSERVATION_LEVEL_IDENTIFIED){
			...
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"target has already been identified, priority " + priority );
			}
			if (priority > 0){
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.FINE,"weighted range = " + distance );
		}
		return distance;
	}
	...
	private void addTargets(SortedList sortedList, Vector<EntityDetection> previousList){
		...
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSE,"maximum number of targets for list " + 
		...
		for (int i=0;i<sortedList.size();i++){
			...
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.COURSE,"attempting to add target " + target.getName() );
			}
			for (EntityDetection detection: previousList){
				if (detection.getEntity() == target){
					if (tracing){
						Tracer.write(Tracer.DETECTION,Tracer.COURSE,"target was on previous list ");
					}
			...
			if ((targets.getSize() + previousList.size())< 
				...
				targets.add(target);
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.FINER,"added " );
				}
			} else {
				if (tracing){
					Tracer.write(Tracer.DETECTION,Tracer.FINER,"target list full " );
				}
			}
		}
		if (tracing){
			Tracer.write(Tracer.DETECTION,Tracer.COURSE,"adding remaining previous targets ");
		}
		for (EntityDetection detection: previousList){
			DetectedEntity target = detection.getEntity();
			if (tracing){
				Tracer.write(Tracer.DETECTION,Tracer.FINE,"adding " + target.getName() );
			}
			targets.add(target);
		}
